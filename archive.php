...
if($taxonomy != 'product-type' && $taxonomy != 'application-area' && $taxonomy != 'application-surface' && $taxonomy != 'finish'){
?>

	<main id="primary" class="site-main">

		<div class="pro-category pat-80 pab-80">
			<div class="small-container">
				<?php if ( have_posts() ) : ?>

					<header class="page-header text-center">
						<?php
						the_archive_title( '<h1 class="page-title font-32">', '</h1>' );
						the_archive_description( '<div class="archive-description">', '</div>' );
						?>
					</header><!-- .page-header -->
					<div class="pro-cate-list">
						<div class="pro-cate-inlist d-flex flex-wrap">
							<?php
							/* Start the Loop */
							while ( have_posts() ) :
								the_post();
...
<?php
	        ...
	        <form>
	            <div class="color-main-filter">
	                ...
	                <div class="filter-list-main">
	                    ...
	                    if (!empty($terms)) { ?>
	                        <div class="dropdown">
	                            <a id="cat-application-area" href="javascript:void(0)" class="js-link">Application Area <i class="fa fa-chevron-down"></i></a>
	                            <ul class="js-dropdown-list checkbox-container">
	                            <?php
	                            foreach ($terms as $term) {
	                                $term_id = $term->term_id;
	                                $term_name = $term->name; ?>
	                                <li id="application-area-<?php echo $term_id; ?>">
	                    ...
	                    if (!empty($prtypes)) { ?>
	                        <div class="dropdown">
	                            <a id="cat-product-type" href="javascript:void(0)" class="js-link1">Product Type <i class="fa fa-chevron-down"></i></a>
	                    ...
	                    if (!empty($surfaces)) { ?>
	                        <div class="dropdown">
	                            <a id="cat-application-surface" href="javascript:void(0)" class="js-link2">Application Surface <i class="fa fa-chevron-down"></i></a>
	                            <ul class="js-dropdown-list2 checkbox-container">
	                            <?php
	                            foreach ($surfaces as $surface) {
	                                $surface_id = $surface->term_id;
	                                $surface_name = $surface->name; ?>
	                                 <li id="application-surface-<?php echo $surface_id; ?>">
	                    ...
	                    if (!empty($finishs)) { ?>
	                    <div class="dropdown">
	                        <a id="cat-finish" href="javascript:void(0)" class="js-link3">Finish <i class="fa fa-chevron-down"></i></a>
	                        <ul class="js-dropdown-list3 checkbox-container">
	                             <?php
	                            foreach ($finishs as $finish) {
	                                $finish_id = $finish->term_id;
	                                $finish_name = $finish->name; ?>
	                                 <li id="finish-<?php echo $finish_id; ?>">
	            ...
	            <div class="selected-options ppf" id="selectedOptionsContainer">
	            	<?php if($termid){
	            		$termslt = get_term($termid, $taxonomy);
	            		
	            		$termsltname = $termslt->name;
	            		if($termsltname){
	            			echo '<span>'.$termsltname.'<span class="close-icon"> × </span></span>';
	            		}
	...
	<section class="bottom-arc-product arcglbpg">
	    <div class="max-container">
	        ...
	        <?php 
	            ...
	            if ($taxonomy === 'application-area') {
	                $appterms[] = $termid;
	            } elseif ($taxonomy === 'product-type') {
	                $typeterms[] = $termid;
	            } elseif ($taxonomy === 'application-surface') {
	                $surfaceterms[] = $termid;
	            } elseif ($taxonomy === 'finish') {
	                $finishterms[] = $termid;
	            }
	...
	<script type="text/javascript">
	    ...
	    jQuery(document).ready(function() {
	    	var catname = '<?php echo $taxonomy; ?>';
	    	var catval = '<?php echo $termid; ?>';
	    	//jQuery('#cat-'+catname).trigger('click');
	    	jQuery('input[type="checkbox"][value="' + catval + '"]').prop('checked', true).change();
