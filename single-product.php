...
$fields = CFS()->get( 'vibrant_colours' );
if (!empty($fields)) {
?>
	<section class="new-living-space">
		<div class="new-living-space-image">
			<?php
			foreach ( $fields as $field ) {
				echo '<link rel="prefetch" as="image" href="' . $firstField['color_image'] . '">';
			}
				if (!empty($fields)) {
					$firstField = $fields[0];
					echo '<img  class="new-living-space-imageSrc" src="' . $firstField['color_image'] . '" alt="' .  $firstField['color_name'] . '">';
				}
			?>
		</div>
		<div class="new-living-space-content">
		<div class="lspace-content">
			<h3><?php echo $background_title; ?></h3>
			<?php echo $background_description; ?>
		</div>
		<div class="new-living-space-filer-cover ">
			<ul class="new-living-space-filter living-space-filter-carousel owl-carousel nav-style">
				<?php
				$count = 1;
					foreach ( $fields as $field ) {
						$image = $field['color_image'];
						 	$color = $field['color_hex'];
						 	$colortitle = $field['color_name'];
						 	$colorcode = $field['color_code']; ?>

					<li class="item <?php if ($count == 1) {
									echo 'active';
...
$inspiration_slider_option = get_field('often_slider');
    	...
    	if ($inspiration_slider_option === 'manuals') {
			...
			<div class="often-slide owl-carousel nav-style"><?php
				foreach ($manual_posts as $post) {
					...
					<div class="item">
						<a href="<?php echo get_the_permalink($post->ID); ?>" class="often-inner text-center">
...
jQuery(document).ready(function ($) {
		...
		$(".living-space-filter-carousel").owlCarousel({
			loop: true,
			margin: 12,
			nav: true,
			dots: false,
			navText: [
				'<svg xmlns="http://www.w3.org/2000/svg" width="41" height="41" fill="none" viewBox="0 0 41 41"> <circle cx="20.5" cy="20.5" r="20.5" fill="#F0F0F0" transform="matrix(-1 0 0 1 41 0)"/> <path fill="#838383" d="M26.465 10.462c.342.296.535.698.535 1.116 0 .42-.193.82-.535 1.117l-9.051 7.814 9.05 7.815c.334.298.518.697.514 1.11a1.48 1.48 0 0 1-.536 1.104 1.983 1.983 0 0 1-1.277.462 1.992 1.992 0 0 1-1.286-.443l-10.344-8.931A1.478 1.478 0 0 1 13 20.509c0-.418.193-.82.535-1.116l10.344-8.93A1.985 1.985 0 0 1 25.172 10c.485 0 .95.166 1.293.462Z"/> </svg>',
				'<svg xmlns="http://www.w3.org/2000/svg" width="41" height="41" fill="none" viewBox="0 0 41 41"><circle cx="20.5" cy="20.5" r="20.5" fill="#F0F0F0"/><path fill="#838383" d="M14.535 10.462A1.478 1.478 0 0 0 14 11.578c0 .42.193.82.535 1.117l9.051 7.814-9.05 7.815a1.472 1.472 0 0 0-.514 1.11c.004.415.197.81.536 1.104.339.292.797.458 1.277.462.48.003.941-.156 1.286-.443l10.344-8.931c.342-.296.535-.698.535-1.117 0-.418-.193-.82-.535-1.116l-10.344-8.93A1.985 1.985 0 0 0 15.828 10c-.485 0-.95.166-1.293.462Z"/></svg>'
			],
			navContainer: '.new-living-space-filer-cover .custom-nav',
			responsive: {
				0: {
					items: 3
				},
...
<script>
jQuery(document).ready(function ($) {
    $('.new-living-space-filter li').click(function() {
        $('.new-living-space-filter li').removeClass('active');
        $(this).addClass('active');
        var imageUrl = $(this).attr('image-url');
        var altText = $(this).find('p').first().text(); // Extracting text from the first <p> inside the clicked li
        $('.new-living-space-imageSrc').attr('src', imageUrl).attr('alt', altText);
    });
});
